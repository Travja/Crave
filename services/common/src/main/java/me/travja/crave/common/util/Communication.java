...
public class Communication {
    ...
    public static <T> T request(String service, String path, String authorization,
        ...
        ServiceInstance itemService = loadBalancerClient.choose(Services.ITEM_SERVICE);
        if (itemService == null) { // Can't authenticate... just go through I guess
            System.err.println("Couldn't find " + service + "!");
            return null;
        }

        return request(itemService.getUri() + "/receipt", authorization, method, body, responseClass);
    }
